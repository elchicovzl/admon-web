...
import { getUsersCount } from '@/lib/actions/user.actions'
...
// Separate async component for user stats
...
// Separate async component for assignments widget
async function MyAssignmentsWidget() {
  const myAssignmentsStatsResult = await getMyAssignmentsStats()
  const myAssignmentsStats = myAssignmentsStatsResult.success ? myAssignmentsStatsResult.data : null

  if (!myAssignmentsStats || myAssignmentsStats.total === 0) {
    return null
  }
...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* User Stats - progressive rendering */}
      <Suspense fallback={<DashboardStatsSkeleton />}>
        <UserStats />
      </Suspense>

      {/* My Assignments Widget - progressive rendering */}
      <Suspense fallback={<MyAssignmentsWidgetSkeleton />}>
        <MyAssignmentsWidget />
      </Suspense>

      {/* User info and quick links - render immediately */}
